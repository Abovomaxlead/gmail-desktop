...
import type { MessageMeta } from './gmail-api';
import { displayName } from './mail-archive';

/** What Gmail wrote on the notification, before any privacy replacement main applies to
 * the card. The subject is the lead; the sender only settles a tie. */
...
/** Folded the way the notification folds it: Gmail collapses newlines and runs of spaces
 * into one before it draws the text, so a subject that wrapped in the header arrives here
 * on a single line and would never compare equal otherwise. */
...
export function subjectMatches(candidate: string, notified: string): boolean {
  ...
  // Mail with no subject at all: Gmail notifies with an empty body, and main puts "(no
  // subject)" on the card. Matching an empty want against everything would hand back the
  // newest mail in the inbox regardless of what was clicked, so it matches only the mail
  // that has no subject either.
  if (!want) return !have;
...
/** Gmail's own "Re:"/"Fwd:" markers, in the languages the app runs in. The title of an
 * open conversation carries the thread's subject as the header shows it, which is without
 * them; a notification about a reply carries them. Stripped from both sides so the two can
 * be compared at all. */
const REPLY_PREFIX = /^((re|aw|fwd?|antw|doorst)\s*(\[\d+\])?\s*:\s*)+/i;

/** Whether the page's title says this conversation is the one on screen.
 *
 * The title is the only thing about a Gmail view that says which mail it is *showing*, as
 * opposed to which mail it was *asked* for. The hash is no answer to that: the app writes
 * the hash itself, so it reads back as the target the instant it is set, while Gmail is
 * still fetching — and everything on screen, including the pop-out button, still belongs to
 * the conversation that was open before.
 *
 * Anchored at the front and stopped at Gmail's " - " separator, never a bare prefix: the
 * title of "Kennissessies september" starts with the subject "Kennissessies", and treating
 * that as a match is exactly the confusion this exists to prevent. A subject Gmail cut
 * short has nothing but its prefix left and is matched on that, since something is better
 * than refusing every long subject. */
...
/** The message the notification was about, or null when nothing in `metas` can be it.
 *
 * Null is a real answer and not a failure: the mail may have been moved, or the account
 * may be busy enough that it already fell outside the handful of messages fetched. The
 * caller falls back to the page's own lookup, which is where this started.
 *
 * When several mails share a subject the sender decides, and when that decides nothing
 * either the newest wins — a notification is about mail that just arrived, so of two
 * candidates it is the later one. */
